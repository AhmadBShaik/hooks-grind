...
export const WithoutUseMemoSample1: React.FC = () => {
  const [num1, setNum1] = React.useState<number>(1);
  const [num2, setNum2] = React.useState<number>(1);
  const computedValue = timeTakingFunc(num1);
  return (
    <div>
      <h1>computation without useMemo hook</h1>

      <div>result:{computedValue}</div>
      <div>{num1} (takes some time to update)</div>
      <div>{num2} (takes some time to update)</div>
      <div>
        <button
          style={{ margin: 5 }}
